...
class SearchHandler(object):
    ...
    def __init__(self, conn, relpath="/select", wt='xml'):
        self.conn = conn
    ...
    def __call__(self, q=None, fields=None, highlight=None,
        ...
        Returns a Response instance id wt=xml (default), returns python dict
        if wt=python or wt=json, returns the raw data if wt is none of the 
        above.
        """
        ...
        if self.wt == 'xml':
            return parse_query_response(StringIO(data),  params, self)
...
class ResponseContentHandler(ContentHandler):
    ...
    def endElement(self, name):
        ...
        if name == 'int':
            ...
            node.final = dict(
                    [(cnode.attrs.get('name', 'unknown'), cnode.final)
